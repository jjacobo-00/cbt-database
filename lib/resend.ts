...
export async function sendOtpEmail(email: string, otp: string, memberName?: string) {
  ...
  const fromEmail = process.env.RESEND_FROM_EMAIL || 'CBT Database <onboarding@resend.dev>'

  if (!apiKey) {
    console.log(`\n========================================`)
    console.log(`[DEV OTP MODE] RESEND_API_KEY not set in environment.`)
  ...
  try {
    const { data, error } = await resend.emails.send({
      from: fromEmail,
      to: [email],
      subject: `${otp} is your CBT Member Portal login code`,
      html: `
        <!DOCTYPE html>
        <html>
        <head>
    ...
    throw new Error(err.message || 'Error sending OTP email')
  }
}
