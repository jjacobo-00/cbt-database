...
export default function StewardshipLoading() {
  return (
    <div className="space-y-6 max-w-7xl mx-auto pb-12 animate-in fade-in duration-300">
      ...
      <div className="grid gap-6 md:grid-cols-2">
          ...
          <CardContent className="h-[280px] flex items-end gap-3 pt-6 px-4">
            {Array.from({ length: 6 }).map((_, j) => (
              <div key={j} className="flex-1 flex flex-col items-center gap-2">
                <Skeleton className="w-full rounded-t-lg" style={{ height: `${35 + (j * 15) % 60}%` }} />
        ...
        {/* Giving Type Distribution Donut Chart */}
        <Card className="rounded-2xl shadow-xs">
          <CardHeader className="space-y-1.5 pb-2">
          ...
          <CardContent className="h-[280px] flex flex-col justify-between py-4 px-2 space-y-2">
            {Array.from({ length: 5 }).map((_, k) => (
              <div key={k} className="flex items-center gap-3">
                ...
                <Skeleton className="h-5 rounded-r-md flex-1" style={{ width: `${30 + (k * 15) % 65}%` }} />
              </div>
            ))}
          </CardContent>
        </Card>

        {/* Tenure Analysis Donut Chart */}
        <Card className="rounded-2xl shadow-xs">
          <CardHeader className="space-y-1.5 pb-2">
