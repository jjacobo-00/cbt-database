...
export default function MinistryCapacityLoading() {
  return (
          ...
          <CardContent className="h-[280px] flex flex-col justify-between py-4 px-2 space-y-2">
            {Array.from({ length: 6 }).map((_, j) => (
              <div key={j} className="flex items-center gap-3">
                ...
                <Skeleton className="h-5 rounded-r-md flex-1" style={{ width: `${40 + (j * 12) % 55}%` }} />
              </div>
            ))}
          </CardContent>
        </Card>

        {/* Age Demographics Full Width Bar Chart */}
          ...
          <CardContent className="h-[260px] flex items-end gap-4 pt-6 px-4">
            {Array.from({ length: 5 }).map((_, k) => (
              <div key={k} className="flex-1 flex flex-col items-center gap-2">
                <Skeleton className="w-full rounded-t-lg" style={{ height: `${30 + (k * 20) % 65}%` }} />
